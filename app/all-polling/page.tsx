...
function AllPolling() {
  const [polling, setPolling] = useState<{ _id: string }[]>([]);
    ...
    if (!token) {
      ...
      axios
        ...
        .then((res) => {
          if (res.status === 200) {
            axios
              .get(SERVER_URL + "/admin/poling-party?page=1&perPage=10", {
                headers: {
                  "x-access-token": token,
                },
              })
  ...
  const handleSearch = () => {
    ...
    if (page > 0 && page <= totalPage) {
      axios
        .get(
          SERVER_URL + "/admin/poling-party?page=1&perPage=10&search=" + search,
          {
            headers: {
              "x-access-token": token,
            },
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    axios
      .get(SERVER_URL + "/admin/poling-party?page=" + page + "&perPage=10", {
        headers: {
          "x-access-token": token,
        },
      })
  ...
  const handleDelete = (id: string) => {
    ...
    axios.delete(
      ...
      .then((res) => {
        setPolling(polling.filter((user) => user._id !== id));
      })
      .catch((err) => {
        console.log(err);
      });
...
const handleFilteredSearch = () => {
  ...
  axios.get(`${SERVER_URL}/admin/poling-party?page=1&perPage=10${query}`, {
    headers: {
      "x-access-token": token,
    }
  }).then((res) => {
      ...
      <Sidebar>
        ...
        <div className="max-w-sm mx-auto mt-4">
          <p
            ...
            className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
          >
           Total Number of Polling Groups : {totalCount}
          </p>
      
        </div>
        
        <h1 className="text-3xl font-bold my-5">All Polling</h1>
